...
app.post("/memories", requireAuth, async (req, res) => {
  ...
  const result = await pool.query(
    ...
    [ts, type, JSON.stringify(tags), normText, meta ? JSON.stringify(meta) : "{}", embedding ? JSON.stringify(embedding) : null, source]
  );
...
app.get("/memories/:id", requireAuth, async (req, res) => {
  ...
  res.json(stripEmbeddings(rows)[0]);
});
...
app.get("/memories/by-tag", requireAuth, async (req, res) => {
  ...
  try {
    ...
    res.json({ ok: true, count: rows.length, memories: stripEmbeddings(rows) });
  } catch (err) {
    console.error("by-tag query failed:", err);
    res.status(500).json({ error: "db query failed" });
  }
});

// New: today's memories
app.get("/memories/today", requireAuth, async (req, res) => {
  try {
    const { rows } = await pool.query("SELECT * FROM memories WHERE created_at::date = CURRENT_DATE ORDER BY id DESC");
    res.json({ ok: true, count: rows.length, memories: stripEmbeddings(rows) });
  } catch (err) {
...
app.get("/ping", async (req, res) => {
  try {
    ...
    res.json({ ok: true, message: "pong", db: "connected" });
  } catch (err) {
